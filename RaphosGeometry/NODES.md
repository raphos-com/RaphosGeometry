...
| **RANSAC Shape Detection** | Detect multiple primitives (planes, spheres, cylinders) in a cloud. Per point it returns **Labels** (0,1,2… = the 1st, 2nd, 3rd… primitive found, -1 = unassigned) and **Shape Type** ("Plane"/"Sphere"/"Cylinder"/"Unassigned" — what each label is), plus **Primitive Count**. | Reverse-engineering a mechanical scan into CAD primitives, or segmenting a building facade / piping run into planes and cylinders. |
