...
| **Marching Cubes (Field)** | Extract an isosurface from a Synera spatial field (`ISpatialField`): connect a field (a distance field, Raphos Tools' Surface Curvature Field, an FEA result field), give a box + resolution, and it samples the field and meshes the level set. | Meshing an implicit shape, an SDF offset, or field/simulation data directly, without hand-building sample values. |
...
| **RANSAC Shape Detection** | Detect multiple primitives (planes, spheres, cylinders) in a cloud. Per point it returns **Labels** (0,1,2… = the 1st, 2nd, 3rd… primitive found, -1 = unassigned) and **Shape Type** ("Plane"/"Sphere"/"Cylinder"/"Unassigned" — what each label is); it also returns the **Fitted Shapes** — the actual recovered solids (a trimmed plane patch, sphere or cylinder per primitive, built with the geometry kernel and bounded to its inliers) — plus **Primitive Count**. | Reverse-engineering a mechanical scan into CAD primitives you can measure/boolean/export, or segmenting a building facade / piping run into planes and cylinders. |
